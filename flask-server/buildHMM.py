...
def solHMM(seed):
    bulletpoint = ""
    transfile = open('transition.txt', 'r')
    emisfile = open('emission.txt', 'r')
    transition = json.loads(transfile.read())
    emission = json.loads(emisfile.read())
    if not seed:
        prior = priorDict()
        ...
        bulletpoint+=word
        tag = pos_tag(word_tokenize(word), tagset='universal')[0][1]
...
if __name__ == "__main__":
    ...
    transfile = open('transition.txt', 'w')
    transfile.write(json.dumps(transition))
    emission = buildEmission()
    emisfile = open('emission.txt', 'w')
    emisfile.write(json.dumps(emission))
